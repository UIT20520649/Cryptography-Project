...
void Encryption(string &plain,const SecByteBlock &key, const SecByteBlock &iv){
...
void Decryption(string &cipher, const SecByteBlock &key, const SecByteBlock &iv){
...
int main(int argc, char* argv[])
{
	...
	string input ,cipher, encoded, recovered;
    ...
    for(;;){
        ...
        if(choice == "e"){
                ...
                Encryption(input,key,iv);
        }
        else if (choice == "d")
        {
            CBC_Mode<AES>::Decryption d;
            input = InputFromScreen();
            StringSource ss(encoded, true, new StreamTransformationFilter(d, new StringSink(input)));
            Decryption(input,key,iv);
        }   
        else{

                wcout << "Invalid choice!\n";
                continue;
        }
        break;
    }
